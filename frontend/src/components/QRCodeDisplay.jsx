...
import { downloadSvgAsPng, slugifyFilename } from '../utils/downloadQR';
 ...
 * changes. Uses SVG representation for sharp vector rendering and offscreen
 * canvas generation for uncorrupted high-resolution downloads.
 */
export default function QRCodeDisplay({ shortUrl, title, size = 180 }) {
  ...
  const handleDownload = () => {
    ...
    downloadSvgAsPng(svg, `${slugifyFilename(title)}-qr`, 1024); // 1024px for premium print resolution
  };
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      >
        <QRCodeSVG value={shortUrl} size={size} level="M" includeMargin={false} />
      </div>
      <button
        type="button"
